...
import { X, Loader2, Zap, Globe, Twitter, Send, Upload, AlertCircle, Plus, Minus, ExternalLink, Key, Eye, EyeOff } from 'lucide-react';
...
export function VampModal({ onClose, onOpenPortfolio }: VampModalProps) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(fetchTokenInfo, 600);
      ...
      for (let i = 0; i < launches; i++) {
        ...
        const createRes = await fetch('/api/solana/create-token', {
          ...
          body: JSON.stringify({
            ...
            name: form.name, symbol: form.symbol, uri,
            buyAmountSol: buyAmt, mintPrivateKey: finalMintKey,
          }),
        });
          ...
          <div className="p-7">
              ...
              <div className="space-y-5">
                ...
                <div className="p-5 rounded-2xl border border-red-500/30 bg-red-500/5">
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Social icons — now clickable <a> tags */}
                      <div className="flex gap-3 mt-2">
                        {form.website && (
                          <a href={form.website} target="_blank" rel="noopener noreferrer"
                            className="text-white/40 hover:text-white transition-colors" title={form.website}>
                            <Globe className="w-4 h-4" />
                          </a>
                        )}
                        {form.twitter && (
                          <a href={form.twitter} target="_blank" rel="noopener noreferrer"
                            className="text-white/40 hover:text-white transition-colors" title="Twitter/X">
                            <Twitter className="w-4 h-4" />
                          </a>
                        )}
                        {form.telegram && (
                          <a href={form.telegram} target="_blank" rel="noopener noreferrer"
                ...
                <div>
                  ...
                  <div className="flex gap-2 mb-3">
                    {([
                      ...
                      { id:'custom', icon:'✨', label:'Custom CA',      sub:'Your vanity keypair', tip:'Paste the private key of any pre-generated vanity mint keypair.' },
                    ] as const).map(lt => (
                      <div key={lt.id} className="relative flex-1 group">
                        <button onClick={() => { setLaunchType(lt.id as any); setMintPrivateKey(''); setMintPreviewCA(''); }}
                          ...
                          <div className="w-4 h-4 rounded-full border border-white/20 text-white/30 text-[9px] flex items-center justify-center flex-shrink-0 cursor-help">?</div>
                        </button>
                  ...
                  {launchType==='custom' && (
                    <div className="p-4 rounded-xl border border-yellow-400/30 bg-yellow-400/5 space-y-3">
                      ...
                      <div className="relative">
                        <input value={mintPrivateKey} onChange={e => { setMintPrivateKey(e.target.value); setMintPreviewCA(''); }}
                          placeholder="Paste mint private key (base58)…" type={showMintKey ? 'text' : 'password'}
                ...
                <div className="p-4 rounded-2xl border border-white/10 bg-white/3 space-y-4">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-white/10">
                    <span className="text-sm text-white/50">{launches} × {devBuyNum > 0 ? `${devBuySOL} SOL` : '0 SOL'}</span>
                    <span className="font-black text-white">= <span className="text-red-400">{totalSOL} SOL</span></span>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <Button onClick={handleLaunch} disabled={!selectedWallet || isLaunching}
                    ...
                    {isLaunching ? (
                      ...
                      <><Zap className="w-4 h-4 mr-2" />{launches > 1 ? `LAUNCH × ${launches}` : 'FAST LAUNCH'}</>
                    )}
                  </Button>
                </div>
              </div>
            ...
            {step === 'launch' && (
              <div className="text-center py-6 space-y-3">
                ...
                {launchedMint && <p className="text-xs text-white/25 font-mono break-all px-2">{launchedMint}</p>}
                ...
                <div className="flex flex-col gap-2 pt-1">
                  <Button onClick={onClose} className="w-full bg-red-500 hover:bg-red-600 text-white font-bold h-11">Close</Button>
