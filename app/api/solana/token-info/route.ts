...
export async function GET(req: NextRequest) {
  const ca = req.nextUrl.searchParams.get('ca')?.trim().replace(/pump$/i,'').trim();
  ...
  // 3. Helius + IPFS fallback only if still missing
  if (!name || !image) {
    try {
      ...
      if(ar.ok){
        ...
        if(jsonUri&&(!description||!twitter)){
          ...
          if(hash&&(jsonUri.includes('ipfs.io')||jsonUri.includes('ipfs://'))){
            gateways.unshift(`https://cloudflare-ipfs.com/ipfs/${hash}`,`https://gateway.pinata.cloud/ipfs/${hash}`);
          }
