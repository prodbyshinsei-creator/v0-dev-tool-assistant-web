...
export async function GET(req: NextRequest) {
  ...
  // ALL 5 in parallel — including Helius to get ipfsUri immediately
  const [pumpR, pump2R, dexR, gmgnR, heliusR] = await Promise.allSettled([
    fetch(`https://frontend-api.pump.fun/coins/${ca}`, { headers: HDR, signal: AbortSignal.timeout(3000) })
      .then(r => r.ok ? r.json() : null).catch(() => null),
    fetch(`https://client-api-2-74b1891ee9f9.herokuapp.com/coins/${ca}`, { headers: HDR, signal: AbortSignal.timeout(4000) })
      .then(r => r.ok ? r.json() : null).catch(() => null),
  ...
  if (pumpR.status === 'fulfilled' && pumpR.value) {
    const p = pumpR.value;
    name=p.name||''; symbol=p.symbol||''; image=p.image_uri||p.image||'';
  ...
  if (pump2R.status === 'fulfilled' && pump2R.value) {
    const p = pump2R.value;
    if(!name)        name        = p.name        || '';
    ...
    if(!twitter && p.twitter)  twitter  = p.twitter.startsWith('http')  ? p.twitter  : `https://x.com/${p.twitter.replace('@','')}`;
    if(!telegram && p.telegram) telegram = p.telegram.startsWith('http') ? p.telegram : `https://t.me/${p.telegram.replace('@','')}`;
  ...
  if (dexR.status === 'fulfilled' && dexR.value) {
    ...
    if (pair) {
      ...
      liquidity=pair.liquidity?.usd||0; volume24h=pair.volume?.h24||0;
  ...
  if (heliusR.status === 'fulfilled' && heliusR.value) {
    ...
    ipfsUri=content.json_uri||'';
    if(!name)   name   = meta.name   || '';
    if(!symbol) symbol = meta.symbol || '';
    if(!image && files[0]) image=files[0].uri||files[0].cdn_uri||'';
  }

  // IPFS JSON fetch — most reliable source for description
  if (ipfsUri && !description) {
    const gateways=[ipfsUri];
    const hash=ipfsUri.split('ipfs/').pop();
    if(hash&&(ipfsUri.includes('ipfs')||ipfsUri.startsWith('ipfs://'))){
      gateways.unshift(
